...
| Area | State |
|---|---|
| Controller RTL | DDR3-800 init sequencer, temporary single-bank command slices, one-bank row/timing machine, and the first global scheduler timing slice; no controller-owned PHY |
| Formal | Live command timing monitor self-check, init sequencer proof, single-read proof, single-write/read proof, bank-machine proof, and scheduler global-timing proof |
...
- `formal` proves the reusable DDR3 command timing monitor against a legal
  ...
  slice for bounded traffic on two active banks.
...
| `rtl/ddr3_scheduler.sv` | First global command scheduler slice for cross-bank timing gates. |
...
| `formal/scheduler_wrapper.sv` | Formal harness for the scheduler global timing slice. |
