// Dual-channel Wishbone-to-DDR3 BL8 line bridge.
//
// This is the bus-facing bridge a real DQ/DQS PHY should consume. Unlike the
// older packetized compatibility path, each write request presents the complete
// 64-byte BL8 channel line before the scheduler accepts the command. Reads
// return one complete BL8 line after the scheduler reaches the RD data window.
...
module ddr3_wb_dual_channel_line #(
    ...
    ddr3_wb_line_channel #(
        ...
        .i_phy_rd_line_valid(i_phy_rd_line_valid[0]),
        .i_phy_rd_line_data(i_phy_rd_line_data[0 +: LINE_DATA_W]),
        .i_phy_rd_line_err(i_phy_rd_line_err[0])
    );
    ...
    ddr3_wb_line_channel #(
        ...
        .i_phy_rd_line_valid(i_phy_rd_line_valid[1]),
        .i_phy_rd_line_data(i_phy_rd_line_data[LINE_DATA_W +: LINE_DATA_W]),
        .i_phy_rd_line_err(i_phy_rd_line_err[1])
    );

    wire _unused = &{1'b0, decoded_line_addr_unused,
                     decoded_word_index_unused, decoded_bank_unused,
                     decoded_row_unused, decoded_col_line_unused,
                     decoded_col_unused, 1'b0};
endmodule
