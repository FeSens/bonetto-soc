// Minimal single-bank write/read command sequencer.
//
// This slice still does not implement a PHY. It emits the DDR3 command order
// needed for one same-row WRITE followed by one READ so the Micron bench can
// validate real x8 memory contents with an ideal DQS/DQ agent.

`default_nettype none
`include "ddr3_params.vh"

module ddr3_single_write_read_seq #(
    ...
    parameter integer T_WTR_CMD = `DDR3_800_TWTR_CMD_CYCLES,
    ...
    parameter [ADDR_BITS-1:0] WR_COL = 15'h0000,
    parameter [ADDR_BITS-1:0] RD_COL = 15'h0000
) (
    input wire                 i_clk,
    input wire                 i_rst,
    input wire                 i_start,
    ...
    always @(posedge i_clk) begin
        if (i_rst) begin
            ...
            case (o_state)
                ...
                ST_WRITE: begin
                    set_cmd(`DDR3_CMD_WR, BANK, WR_COL);
                    o_state   <= ST_WAIT_WTR;
                    wait_left <= load_wait(T_WTR_CMD);
                end
                ...
                ST_READ: begin
                    set_cmd(`DDR3_CMD_RD, BANK, RD_COL);
                    o_state   <= ST_WAIT_RTP;
