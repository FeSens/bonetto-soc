...
module tb_micron_ctrl_dual_channel;
    ...
    localparam [WB_DATA_W-1:0] CH0_DATA = 32'h0123_4567;
    localparam [WB_DATA_W-1:0] CH1_DATA = 32'h89ab_cdef;
    ...
    task wb_write_word;
        ...
        begin
            ...
            wait (wb_ack);
            if (wb_err) begin
                $display("[micron-ctrl-dual] write ack error channel=%0d addr=%h",
                         channel, addr);
                $fatal(1);
            end
            wait (line_done[channel]);
            finish_wb();
        end
    endtask
    ...
    task wb_read_word;
        ...
        begin
            ...
            if (wb_err || wb_dat_r !== expected) begin
                $display("[micron-ctrl-dual] read mismatch addr=%h expected=%h got=%h err=%0b",
    ...
    initial begin
        ...
        $display("[micron-ctrl-dual] dual-channel full-width controller Micron loopback passed");
        $finish;
    end

    initial begin
        repeat (12000) @(posedge ck);
        $display("[micron-ctrl-dual] timeout init_done=%b ack=%0b stall=%0b line_done=%b line_rd=%b",
                 init_done, wb_ack, wb_stall, line_done, line_rd_valid);
        $fatal(1);
    end
endmodule
