...
- Rerouting the current controller-input-buffer keeper JSON with nextpnr's
  ...
  DDR3-1600 clocks. The committed router1 Make target now routes at a 208 MHz
  system-clock target, reproduces the route end-to-end, and generates
  `build/bonetto_soc_ypcb00338_full_2ch_ddr1600_serdescmd_jtagonly_reqbuf_rmwpatch_ctrlreq_topreq_router1_lateflat.bit`.
  The verified target completed with 1 warning and 0 errors, giving 173.31 MHz
  `u_blu.i_clk_50`, 208.29 MHz `clk_sys`, 966.18 MHz `clk_dq`, and
  1557.63 MHz `clk_phy_x4`; this is the first evidence of system-clock margin
  at the 207.5 MHz point implied by DDR3-1660. The earlier 200 MHz router1
  route had 211.77 MHz `clk_sys`, so the 208 MHz retarget spends most of the
  spare system-clock margin but keeps the route legal. The critical
  `clk_sys` path remains a long runtime-to-PHY write-data route
  (`phy_wr_data[152]`) but now has enough margin at the tested target. Device
  usage for the routed design remained modest: 2237 `SLICE_LUTX`, 3340
  `SLICE_FFX`, 16 `RAMB36E1`, 178 `OSERDESE2`, 16 `IDELAYE2`, 6 `IDELAYCTRL`,
  and 22 `BUFGCTRL`. Treat this as a timing breakthrough, not hardware signoff:
...
| Dual-channel DDR3-1600 router1 timing diagnostic | `make -C boards/ypcb-00338 full-2ch-ddr1600-serdescmd-jtagonly-reqbuf-rmwpatch-ctrlreq-topreq-router1-lateflat-bitstream` recreates the first main-clock passing route at a 208 MHz system-clock target, but uses `--timing-allow-fail` until the board-clock report and lane-local DQS diagnostics are resolved |
