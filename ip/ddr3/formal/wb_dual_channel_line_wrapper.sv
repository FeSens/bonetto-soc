// Formal harness for the line-level dual-channel Wishbone DDR3 bridge.

`default_nettype none

module ddr3_wb_dual_channel_line_wrapper (
    input wire clk,
    input wire rst
);
    ...
    (* anyseq *) wire [(CHANNELS*LINE_DATA_W)-1:0] rd_line_data;
    ...
    ddr3_wb_dual_channel_line dut (
        ...
        .i_phy_rd_line_err({CHANNELS{1'b0}})
    );
...
`ifdef FORMAL
    ...
    always @(posedge clk) begin
        ...
        if (rst || !m_cyc) begin
            ...
            assert(!(wr_line_valid[0] && wr_line_valid[1]));
            assert(!(rd_line_ready[0] && rd_line_ready[1]));
            assert(!s_err);
...
`endif
    ...
    wire _unused = &{1'b0, wr_line_data, wr_line_mask, 1'b0};
endmodule
